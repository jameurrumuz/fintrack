...
function PartyLedgerPage({ params }: { params: Promise<{ partyId: string }> }) {
  ...
  return (
    <SidebarInset className="flex flex-col bg-gray-50 dark:bg-gray-900">
        ...
        {payingInstallment && (
            <PayEmiDialog isOpen={!!payingInstallment} onOpenChange={() => setPayingInstallment(null)} installment={payingInstallment.installment} accounts={accounts} onConfirm={payingInstallment.installment.status === 'paid' ? handleEditEmiPayment : handleMarkEmiAsPaid} onDelete={handleDeleteEmiPayment} />
        )}
        {editingLoan && (
                        ...
                        {party.loans?.map(loan => (
                            <Card key={loan.id} className="bg-muted/50 p-3">
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <div className="flex gap-2">
                                        <Button variant="outline" size="sm" className="h-7 text-[10px]" onClick={() => setEditingLoan(loan)}>Edit</Button>
